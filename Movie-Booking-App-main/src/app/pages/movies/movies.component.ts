  ...
  ngOnInit() : void {
    this.userService.allMovies().subscribe(
      (data:any) =>{
        ...
        while(i <= count){
          this.movies.push({
            position: i, movieName: data[i-1].movieName, theaterName: data[i-1].theatreName, ticketStatus: data[i-1].ticketsStatus,noOfTicketsAvailable: data[i-1].noOfTicketsAvailable
          });
          i++;
        }
